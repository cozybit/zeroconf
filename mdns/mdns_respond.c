...
void responder_tests(void)
{
	...
	FAIL_UNLESS(ret == 0, "Failed to parse pkt6");
	FAIL_UNLESS(VALID_LENGTH(&rx_msg) == sizeof(pkt6),
				"parsed packet has unexpected length (%d != %d)",
				VALID_LENGTH(&rx_msg), sizeof(pkt6));
	DBG("Parsed packet 6:\n");
	debug_print_message(&rx_msg);
}
#endif
