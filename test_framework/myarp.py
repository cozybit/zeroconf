...
class myarp:
	...
	def is_probe(self, mac):
		...
		if self.sender_mac != mac:
			str = "Expected sender mac to be %s but it was %s." % (self.sender_mac, mac)
		if self.target_mac != "000000000000":
			str = "Expected target mac to be 0 but it was %s." % (self.target_mac)
		if self.sender_ip != "0.0.0.0":
			str = "Expected sender ip to be 0 but it was %s." % (self.sender_ip)
